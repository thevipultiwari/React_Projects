...
const App = () => {
  ...
  const getData = async () => {
    const response = await axios.get(
      `https://picsum.photos/v2/list?list?page=${index}&limit=30`
    );
...
<button style={{opacity:index==1?0.5:1}}
        className="bg-amber-400 px-4 py-2 text-sm cursor-pointer text-black rounded font-semibold active:scale-95"
        onClick={() => {
          if (index > 1) {
            setIndex(index - 1);
            setUserData([]);
